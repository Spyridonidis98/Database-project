...
class DataModel():
    ...
    def add_publication_to_magazine(self, mag_issn, volume, issue):
        self.con.execute("insert into PUBLICATION values(?,?,?)",(mag_issn,volume,issue))
        self.con.commit()

    def get_publications_articles(self, mag_issn, volume, issue):
        r = self.con.execute("select * from ARTICLE where Mag_issn=? and Pub_volume=? and Pub_issue=?",(mag_issn, volume, issue))
        d = []
        for i in r:
            d.append(dict(i))
        return d
    ...
    def get_authors(self):
        r = self.con.execute("select * from AUTHOR")
        d = []
        for i in r:
            d.append(tuple(dict(i).values()))
        return d
    ...
    def create_author(self, fname, lname, biography):
        ...
        self.con.execute("insert into AUTHOR values(?,?,?,?,?,?)",(id, fname, lname, biography, 0, 0))
        self.con.commit()
